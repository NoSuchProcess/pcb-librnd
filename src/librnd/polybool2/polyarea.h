 ...
 *
 *  Far descendant of poly_Boolean: a polygon clip library
...
struct rnd_polyarea_s {
	...
	unsigned overlap:1;      /* island-island bbox overlap between A and B */
};
...
typedef enum rnd_poly_bool_op_e {
	...
	RND_PBO_CANON = 200 /* unary operator: convert the polygon to its canonical form; used by polybool2 internally */
} rnd_poly_bool_op_t;
...
/* may modify pl: compute pl->tree if it's NULL */
